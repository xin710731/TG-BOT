# telegram_checkin_multi_lang.py
# 完整版（aiogram 3）：群统一语言（中文 / English / Bahasa Indonesia），由管理员设置
# - 打卡 / 休息（Emoji识别：🚶/🚽/🚬/🍱）/ 回座统计 / 超时提醒
# - 多管理员设置面板（多 ID）
# - 管理员日志（写入 admin_logs）
# - 自动/手动 报表（Excel .xlsx，中文列名，带群名）
# - 自动在首次使用时为群插入 settings 初始行，含 language 字段（默认 zh）
#
# 依赖:
...
dp = Dispatcher()
pending_media_for_chat: Dict[int, str] = {}  # chat_id -> "awaiting_text"/"awaiting_media"
...
LANG_TEXT = {
    "zh": {
        ...
        "choose_lang": "🌐 请选择本群使用的语言 / Choose language / Pilih bahasa :",
        "lang_updated": "🌐 语言已更新。请重新点击菜单按钮使其生效。",
        "choose_lang_btn": "🌐 选择语言 / Choose Language / Pilih Bahasa",
    },
    "en": {
        ...
        "choose_lang": "🌐 Choose language / 请选择语言 / Pilih bahasa :",
        "lang_updated": "🌐 Language updated. Tap the menu again to refresh.",
        "choose_lang_btn": "🌐 Choose Language / 选择语言 / Pilih Bahasa",
    },
    "id": {
        ...
        "choose_lang": "🌐 Pilih bahasa / Choose language / 请选择语言 :",
        "lang_updated": "🌐 Bahasa diperbarui. Ketuk menu lagi untuk memperbarui.",
        "choose_lang_btn": "🌐 Pilih Bahasa / Choose Language / 选择语言",
    }
}
...
def fmt_minutes_lang(m: int, lang: str) -> str:
    if lang == "zh":
        if m >= 60:
            h = m // 60
            mm = m % 60
...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if "language" not in cols:
            await db.execute("ALTER TABLE settings ADD COLUMN language TEXT DEFAULT 'zh'")
        await db.execute("""
...
async def ensure_settings(chat_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if not found:
            await db.execute(
                "INSERT INTO settings (chat_id, reminder_text, reminder_media_file_id, weekly_report_enabled, monthly_report_enabled, language) VALUES (?, ?, ?, 0, 0, 'zh')",
                (chat_id, None, None)
            )
            await db.commit()
...
async def get_chat_settings(chat_id: int):
    await ensure_settings(chat_id)
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT reminder_text, reminder_media_file_id, weekly_report_enabled, monthly_report_enabled, language FROM settings WHERE chat_id = ?", (chat_id,))
        row = await cur.fetchone()
    return {
        "reminder_text": row[0],
        "reminder_media_file_id": row[1],
        "weekly_report_enabled": row[2],
        "monthly_report_enabled": row[3],
        "language": row[4] or "zh",
    }

async def get_chat_language(chat_id: int) -> str:
    settings = await get_chat_settings(chat_id)
    return settings["language"] or "zh"

async def set_chat_language(chat_id: int, lang: str):
    await set_chat_setting(chat_id, "language", lang)
...
def get_admin_menu(lang: str):
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=t["choose_lang_btn"], callback_data="admin:lang")],
...
@dp.message(F.text.func(lambda s: any(e in (s or "") for e in ("🚶", "🚽", "🚬", "🍱"))))
...
async def handler_return_seat(message: types.Message):
    ...
    if lang == "zh":
        msg = (
            ...
            f"📅 今日累计离开 {total_times} 次，共 {fmt_minutes_lang(total_minutes, lang)}\n"
        ...
        msg = (
            ...
            f"📅 Today leaves: {total_times} times, total {fmt_minutes_lang(total_minutes, lang)}\n"
            f"({fmt_hm_local(sdt)} ~ {fmt_hm_local(now)})"
        )
    else:
        msg = (
            ...
            f"📅 Hari ini keluar: {total_times} kali, total {fmt_minutes_lang(total_minutes, lang)}\n"
            f"({fmt_hm_local(sdt)} ~ {fmt_hm_local(now)})"
        )
...
async def handler_today_summary(message: types.Message):
    ...
    if lang == "zh":
        text = (
            ...
            f"{LANG_TEXT[lang]['total_work']}：{fmt_minutes_lang(summary['total_work'], lang)}\n"
            f"{LANG_TEXT[lang]['total_break']}：{fmt_minutes_lang(summary['total_break'], lang)}\n"
            f"{LANG_TEXT[lang]['leave_times']}：{summary['total_leave_times']}\n\n"
            f"{LANG_TEXT[lang]['meal']}：{summary['counts']['meal']} 次（{fmt_minutes_lang(summary['durations']['meal'], lang)}）\n"
            f"{LANG_TEXT[lang]['toilet']}：{summary['counts']['toilet_small'] + summary['counts']['toilet_big']} 次（{fmt_minutes_lang(summary['durations']['toilet_small'] + summary['durations']['toilet_big'], lang)}）\n"
        ...
        text = (
            ...
            f"{LANG_TEXT[lang]['total_break']}: {fmt_minutes_lang(summary['total_break'], lang)}\n"
            f"{LANG_TEXT[lang]['leave_times']}: {summary['total_leave_times']}\n\n"
            f"{LANG_TEXT[lang]['meal']}: {summary['counts']['meal']} ({fmt_minutes_lang(summary['durations']['meal'], lang)})\n"
            f"{LANG_TEXT[lang]['toilet']}: {summary['counts']['toilet_small'] + summary['counts']['toilet_big']} ({fmt_minutes_lang(summary['durations']['toilet_small'] + summary['durations']['toilet_big'], lang)})\n"
    ...
    if not entries:
        ...
        for uid, net_m, break_m in entries[:10]:
            try:
                ...
                name = str(uid)
            if lang == "zh":
                lines.append(f"{pos}. {name} — 工作 {fmt_minutes_lang(net_m, lang)}，休息 {fmt_minutes_lang(break_m, lang)}")
            elif lang == "en":
                lines.append(f"{pos}. {name} — Work {fmt_minutes_lang(net_m, lang)}, Break {fmt_minutes_lang(break_m, lang)}")
            else:
                lines.append(f"{pos}. {name} — Kerja {fmt_minutes_lang(net_m, lang)}, Istirahat {fmt_minutes_lang(break_m, lang)}")
            pos += 1
...
async def admin_set_text(call: types.CallbackQuery):
    lang = await get_chat_language(call.message.chat.id)
...
@dp.message(F.text & F.chat.id.func(lambda cid: cid in pending_media_for_chat))
...
@dp.callback_query(F.data == "admin:lang")
async def admin_choose_language(call: types.CallbackQuery):
    if not is_admin(call.from_user.id):
        return await call.answer("No permission", show_alert=True)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="中文", callback_data="admin:lang:zh")],
        [InlineKeyboardButton(text="English", callback_data="admin:lang:en")],
        [InlineKeyboardButton(text="Bahasa Indonesia", callback_data="admin:lang:id")],
    ])
    lang = await get_chat_language(call.message.chat.id)
    await call.message.edit_text(LANG_TEXT[lang]["choose_lang"], reply_markup=kb)

@dp.callback_query(F.data.startswith("admin:lang:"))
async def admin_set_language(call: types.CallbackQuery):
    lang_code = call.data.split(":")[2]
    await set_chat_language(call.message.chat.id, lang_code)
    await log_admin_action(call.message.chat.id, call.from_user.id, "set_language", f"lang={lang_code}")
    new_lang = await get_chat_language(call.message.chat.id)
    await call.answer("Updated", show_alert=True)
    await call.message.edit_text(LANG_TEXT[new_lang]["lang_updated"], reply_markup=get_admin_menu(new_lang))
...
async def send_report_for_chat(chat_id: int, period: str, base_date: date):
    ...
    for name, start_s, end_s, work_m, break_m, leave_cnt in rows:
        ws.append([
            ...
            start_s,
            end_s,
            fmt_minutes_cn(work_m),
            fmt_minutes_cn(break_m),
            leave_cnt,
            work_m,
            break_m
        ])
    ...
    for admin in ADMIN_IDS:
        try:
            ...
            logging.warning(f"发送报表给管理员 {admin} 失败: {e}")
...
@dp.message(F.text.func(lambda s: s and (("手动发送日报" in s) or ("Send Daily Report" in s) or ("Kirim Laporan Harian" in s))))
